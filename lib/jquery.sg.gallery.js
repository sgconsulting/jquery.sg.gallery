...
(function($){
     ...
     // from http://modernizr.github.com/Modernizr/touch.html
     $.support.touch = (function(){
			    // check if support.touch already exists
			    if ($.support.touch) {
			    ...
			    // try/catch document.createEvent("TouchEvent")
			    try {
	 ...
	 // bind swipe events
	 if ($.support.touch) {
	     if($.mobile) { //jQuery.mobile is being used, so I can simply bind swipe events
		     $(document).bind('swipeleft', function(){advance(1);});
		     $(document).bind('swiperight', function(){retreat(1);});		 
	     } else {
		 // load jquery.mobile.event and bind events
		 $.getScript(
		     'http://c57346.r46.cf1.rackcdn.com/jquery.mobile.event.1.0a4.1.min.js',
		     function() {
			 $(document).bind('swipeleft', function(){advance(1);});
			 $(document).bind('swiperight', function(){retreat(1);});
		     }
	 ...
	 } else { //i'm not a mobile device so I use previous and next buttons to move the gallery
